...
class FirebaseConversationListApi : ConversationList.Api {
    override fun getUserConversationList(): Observable<List<Conversation>> {
        val query = FirebaseDatabase.getInstance()
                .reference
                .child("conversations")
        val mapper = DataSnapshotMapper.of(object : GenericTypeIndicator<HashMap<String, HashMap<String, String>>>() {})
        return RxFirebaseDatabase.observeSingleValueEvent(query, mapper)
                .map { getUserConversations(it) }
    }

    private fun getUserConversations(conversations: HashMap<String, HashMap<String, String>>): List<Conversation> {
